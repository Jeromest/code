...
public class Tetris extends JPanel {
	...
	private int totalScore = 0;//初始化得分
	private int totalLine = 0;//初始化消除行数

	/** 定义三个常量：充当游戏的状态 */
	public static final int PLAYING = 0;//游戏中
	...
	public static BufferedImage T;	//"T"型方块
	public static BufferedImage I;	//"I"型
	public static BufferedImage O;	//"O"型
	public static BufferedImage J;	//"J"型
	public static BufferedImage L;	//"L"型
	public static BufferedImage S;	//"S"型
	public static BufferedImage Z;	//"Z"型
	...
	static {
		try {	//加载图片
			...
			background = ImageIO.read(Tetris.class.getResource("tetris.png"));//读取背景图片
			game_over = ImageIO.read(Tetris.class.getResource("game-over.png"));//读取游戏结束时图片
	...
	private void paintState(Graphics g) {//显示此时游戏状态
	...
	public void paintScore(Graphics g) {//打印分数，消除行数
		g.setFont(new Font(Font.SANS_SERIF, Font.ITALIC, 26));
		g.drawString("得分:" + totalScore, 285, 165);
		g.drawString("消除行数:" + totalLine, 285, 215);
	}
	 ...
	 * @param g
	 */
	public void paintNextOne(Graphics g) {//打印下一个方块
		// 获取nextOne对象的四个元素
	 ...
	 * 
	 */
	public void paintCurrentOne(Graphics g) {//打印此时的方块
	 ...
	 * @param a
	 */
	public void paintWall(Graphics a) {	//打印10*20的方格
		// 外层循环控制行数
		for (int i = 0; i < 20; i++) {
			// 内存循环控制列数
			for (int j = 0; j < 10; j++) {
	...
	public boolean isFullLine(int row) {//判断一行是否已经被填满
		Cell[] line = wall[row];
		for (Cell c : line) {
			if (c == null) {
				return false;
	...
	public boolean canDrop() {//判断是否能继续下落
		Cell[] cells = currentOne.cells;
		/*
		 * 
		 */
	 ...
	 */
	public void landToWall() {//将方块嵌入到墙里面
		Cell[] cells = currentOne.cells;
		for (Cell c : cells) {
			// 获取最终的行号和列号
			int row = c.getRow();
